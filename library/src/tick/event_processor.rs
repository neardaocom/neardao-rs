...
pub trait EventProcessor<E: TickEvent, Q: EventQueue<E>> {
    type ProcessingResult;
    fn get_last_tick(&self) -> TimestampSec;
    fn set_last_tick(&mut self, tick: TimestampSec);
    fn tick_interval(&self) -> TimestampSec;
    fn get_queue(&self, tick: TimestampSec) -> Option<Q>;
    fn remove_queue(&mut self, tick: TimestampSec) -> Option<Q>;
    fn save_queue(&mut self, tick: TimestampSec, queue: &Q) -> Option<Q>;
...
#[cfg(test)]
mod tests {
    use super::{EventProcessor, TickEvent, TimestampSec};
    use crate::tick::event_queue::{EventQueue, EventQueueVec};
    use crate::tick::runner::run_tick;
    ...
    struct ProcessorMock {
        pub event_source: HashMap<TimestampSec, EventQueueVec<Event>>,
    ...
    impl ProcessorMock {
        pub fn new(
            event_source: HashMap<TimestampSec, EventQueueVec<Event>>,
    ...
    impl EventProcessor<Event, EventQueueVec<Event>> for ProcessorMock {
        type ProcessingResult = ();
        ...
        fn get_queue(&self, tick: TimestampSec) -> Option<EventQueueVec<Event>> {
            self.event_source.get(&tick).map(|e| e.to_owned())
        }

        fn remove_queue(&mut self, tick: TimestampSec) -> Option<EventQueueVec<Event>> {
        ...
        fn save_queue(
            &mut self,
            tick: TimestampSec,
            queue: &EventQueueVec<Event>,
        ) -> Option<EventQueueVec<Event>> {
    ...
    macro_rules! make_test_data_and_result {
        ($event_source:expr, $expected_results: expr, $expected_count: literal, $($tick:literal => $($event:ident),*;)*) => {
            $(
                let mut queue = EventQueueVec::new();
                $(
    ...
    fn tick_process_part_queue() {
        ...
        let remaining = run_tick(&mut proc, 4, 20);
    ...
    fn tick_process_one_queue_exact() {
        ...
        let remaining = run_tick(&mut proc, 8, 30);
    ...
    fn tick_process_one_queue_available() {
        ...
        let remaining = run_tick(&mut proc, 8, 15);
        assert_eq!(remaining, 0);
        proc.assert_result(expected_results, "queue one all");
        proc.assert_last_tick(10);
        assert!(proc.get_queue(10).is_none());
    ...
    fn tick_process_one_and_half_queue() {
        ...
        let remaining = run_tick(&mut proc, 12, 40);
    ...
    fn tick_process_two_queues_with_gaps() {
        ...
        let remaining = run_tick(&mut proc, 20, 101);
        assert_eq!(remaining, 0);
        proc.assert_result(expected_results, "queue 1: all");
    ...
    fn tick_flow_scenario() {
        ...
        let remaining = run_tick(&mut proc, 20, 20);
        assert_eq!(remaining, 0);
        ...
        let remaining = run_tick(&mut proc, 20, 30);
        assert_eq!(remaining, 0);
        ...
        let remaining = run_tick(&mut proc, 20, 41);
        assert_eq!(remaining, 0);
        ...
        let remaining = run_tick(&mut proc, 20, 80);
        assert_eq!(remaining, 0);
