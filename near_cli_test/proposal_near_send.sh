...
S_PROPOSE_SETTINGS='{"activity_rights":[[],[{"GroupLeader":1}],[{"GroupLeader":1}]],"activity_inputs":[[],["Free",{"Checked":0}]],"transition_constraints":[[{"transition_limit":1,"cond":null}],[{"transition_limit":4,"cond":null}]],"binds":[{"U128":'$MAX_NEAR'}],"validators":[{"args":[{"User":1},{"Bind":0}],"expr":{"Boolean":{"operators":[{"operands_ids":[0,1],"op_type":{"Rel":"Gt"}}],"terms":[{"Arg":1},{"Arg":0}]}}}],  "storage_key":"wf_send_near_3"}'

PUUID=$(near call $DCID propose '{"template_id":2,"template_settings_id":0,"propose_settings":'$S_PROPOSE_SETTINGS'}' --amount 1 --gas $TGAS_100 --accountId $CID1 | tail -n1 | tr -d '[:space:]')
