use data::workflow::basic::group_package::GroupPackage1;
use near_sdk::ONE_NEAR;

use crate::constants::{
    DAO_TPL_ID_OF_FIRST_ADDED, DAO_TPL_ID_WF_ADD, PROVIDER_TPL_ID_GROUP_PACKAGE1,
};
...
#[tokio::test]
async fn workflow_group_package1() -> anyhow::Result<()> {
    ...
    // Execute AddWorkflow by DAO member to add GroupPackage1 template.
    run_activity(
        &worker,
        &member,
        &dao_account_id,
    ...
    // Propose GroupPackage1.
    let proposal_id = proposal_to_finish(
        &worker,
        &member,
        &dao_account_id,
        DAO_TPL_ID_OF_FIRST_ADDED,
        GroupPackage1::propose_settings(),
        None,
        vec![(&member, 1)],
        100,
        GroupPackage1::deposit_propose(),
        GroupPackage1::deposit_vote(),
        ProposalState::Accepted,
    )
    .await?;
    view_partitions(&worker, &dao_account_id).await?;

    // Execute Workflow GroupPackage1.
    // GroupAdd - artists
    run_activity(
        &worker,
        &member,
        ...
        ActivityInputGroupPkg1::activity_group_add(
            "artists",
            "macho.near",
    ...
    run_activity(
        ...
        ActivityInputGroupPkg1::activity_group_add_members(
            2,
    ...
    run_activity(
        ...
        ActivityInputGroupPkg1::activity_group_remove_members(2, vec!["def.near"]),
        true,
    )
    .await?;
    check_group(
    ...
    run_activity(
        ...
        proposal_id,
        ADMINPACKAGE1_REMOVE_GROUP_ROLES,
        ActivityInputGroupPkg1::activity_group_remove_roles(2, vec![1, 4, 5, 6]),
        true,
    )
    .await?;
    ...
    run_activity(
        ...
        ActivityInputGroupPkg1::activity_group_remove_member_roles(
            2,
    ...
    run_activity(
        ...
        ActivityInputGroupPkg1::activity_group_remove(2),
        true,
    )
    .await?;
