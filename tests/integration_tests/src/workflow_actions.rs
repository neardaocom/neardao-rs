...
use crate::{
    contract_utils::dao::{
        activity_input::{
            run_activity, test_optional_actions::ActivityInputTestOptionalActions as ActivityInput,
        },
        types::consts::PROVIDER_TPL_ID_TEST_OPTIONAL_ACTIONS,
        view::{statistics, workflow_storage_buckets},
    },
...
async fn test_workflow_optional_actions_cannot_leave_activity() -> anyhow::Result<()> {
    ...
    // Finish activity 1 by skiping 2th and 3rd action.
    run_activity(
        &worker,
        &member,
        &dao_account_id,
