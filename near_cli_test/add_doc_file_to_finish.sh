...
PUUID=$(near call $DCID add_proposal '{"proposal_input" : {"description": "Testing doc file.", "tags":["test","doc","files"], "transaction": { "AddDocFile": {"uuid": "5fb2ae525103eb24bb18d1077942666012345678123450","metadata": {"name":"whitepaper","ext": ".pdf", "description":"first whitepaper", "tags": [], "category":0, "valid": true, "v":"1.0" }, "new_tags": ["first", "testing", "important"], "new_category": null} }}}' --amount 1 --gas 100000000000000 --accountId $CID4 | tail -n1 | tr -d '[:space:]')

# Second file with new category and same tags
#PUUID=$(near call $DCID add_proposal '{"proposal_input" : {"description": "Testing doc file.", "tags":["test","doc","files"], "transaction": { "AddDocFile": {"uuid": "5fb2ae525103eb24bb18d1077942666012345678123451","metadata": {"name":"whitepaper","ext": ".pdf", "description":"second whitepaper", "tags": [1,2], "category":0, "valid": true, "v":"1.0" }, "new_tags": [], "new_category": "other"} }}}' --amount 1 --gas 100000000000000 --accountId $CID4 | tail -n1 | tr -d '[:space:]')

# Third file with new category and tags with different lowercase
#PUUID=$(near call $DCID add_proposal '{"proposal_input" : {"description": "Testing doc file.", "tags":["test","doc","files"], "transaction": { "AddDocFile": {"uuid": "5fb2ae525103eb24bb18d1077942666012345678123452","metadata": {"name":"whitepaper","ext": ".pdf", "description":"third whitepaper", "tags": [1], "category":0, "valid": true, "v":"1.0" }, "new_tags": ["IMPORTANT"], "new_category": "governance"} }}}' --amount 1 --gas 100000000000000 --accountId $CID4 | tail -n1 | tr -d '[:space:]')
