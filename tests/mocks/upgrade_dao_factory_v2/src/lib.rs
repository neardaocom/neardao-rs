...
const NEWEST_DAO_VERSION: &[u8] = include_bytes!("../../../res_upgrade/upgrade_dao_v2.wasm");
const MIGRATION_BLOB: &[u8] = include_bytes!("../../../res_upgrade/upgrade_dao_v2_migration.wasm");
        ...
        if r#type != MigrationType::OnlyMigration {
            ...
            if factory.latest_migration_version_idx == factory.latest_upgrade_version_idx {
                env::storage_write(
                    ...
                    &factory.version_count.to_ne_bytes(),
                );
            }
        }
        factory
