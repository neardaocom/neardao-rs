...
PUUID=$(near call $DCID add_proposal '{"proposal_input" : {"description": "I need NEAR TOKENS, bros.", "tags":["test","first","money"], "transaction": { "Pay": {"amount_near": "40000000000000000000000000", "account_id":"'$CID4'"} }}}' --amount 0.5 --gas 100000000000000 --accountId $CID4 | tail -n1 | tr -d '[:space:]')
echo "Created proposal UUID: $PUUID"

near call $DCID vote '{"proposal_id":'$PUUID',"vote_kind": 1}' --gas 10000000000000 --accountId $CID1
near call $DCID vote '{"proposal_id":'$PUUID',"vote_kind": 2}' --gas 10000000000000 --accountId $CID2
